...
#	pragma omp parallel num_threads(thread_count) \
	shared(min_meas,max_meas,thread_count,data_count,bin_count,data,bin_maxes,loc_bin_counts,bin_counts) \
	default(none)
   {
      int j, my_rank = omp_get_thread_num();
      int my_offset = my_rank*bin_count;

#		pragma omp for private(i,my_rank,bin) schedule(static) reduction(+:loc_bin_counts[:thread_count*bin_count])
...
#		pragma omp for private(i,j) schedule(static) reduction(+:bin_counts[:bin_count]) 
...
#  endif
   ...
   /* Print the histogram (UNCOMMENT WHEN NEEDED) */ 
