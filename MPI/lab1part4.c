...
double ping_pong(char mesg[], int mesg_size, int iters, MPI_Comm comm, 
   ...
   if (my_rank == 0) {
#     ifndef CLOCK
      start = MPI_Wtime(); /* start timer for measurement with MPI_Wtime() */
#     else
      start = clock(); /* start timer for measurement with clock() */
#     endif
      for (i = 0; i < iters; i++) {
...
#     ifndef CLOCK
      ...
      printf("Total MPI time = %8.5e seconds\n", MPI_Wtime() - start);  
#     else
      /* return elapsed time as measured by clock() */ 
		printf("Total CLOCK time = %8.5e seconds\n", (double)(clock() - start) / clocks_per_sec);
#     endif
